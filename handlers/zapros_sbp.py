...
async def sbp_cat(callback: CallbackQuery, bot: Bot):
    if not await check_sub(bot, callback.from_user.id): return await callback.answer("❌ Нужна подписка!",
                                                                                     show_alert=True)
...
async def sbp_create(callback: CallbackQuery, bot: Bot):
    ...
    await callback.message.edit_text(f"✅ Заявка #{app_id} создана!", reply_markup=main_menu())