...
async def sdat_request_code(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if not app:
        ...
        return
    if app['status'] in ('cancelled', 'rejected'):
        await callback.message.answer("❌ Заявка отменена или отклонена.")
        return
...
@router.callback_query(F.data.startswith("sdat_reject_"))
async def sdat_admin_reject(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    parts = callback.data.split("_")
    app_id = int(parts[2])
    user_id = int(parts[3])
...
# ---------- Общие обработчики для отмены и отказа ----------
...
async def admin_cancel_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    await bot.send_message(NOTIFY_CHANNEL_ID, f"❌ Отменено (заявка {app_id}). Причина: {reason}")
...
async def admin_reject_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    app = get_app(app_id)
    update_app(app_id, status='rejected', cancel_reason=reason)

    await bot.send_message(user_id, f"❌ Ваша заявка (ID: {app_id}) отклонена.\nПричина: {reason}")
    await bot.send_message(NOTIFY_CHANNEL_ID, f"❌ Отклонено (заявка {app_id}). Причина: {reason}")

    storage_key = StorageKey(bot.id, user_id, user_id)
    await state.storage.set_state(key=storage_key, state=None)
    await state.storage.set_data(key=storage_key, data={})
    await state.clear()
    await message.answer("✅ Отказ выполнен.")
