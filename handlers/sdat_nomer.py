...
async def sdat_phone(message: Message, state: FSMContext, bot: Bot):
    ...
    # Отправляем сообщение в канал с кнопками – оно НЕ будет редактироваться
    try:
...
async def sdat_request_code(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    # Отправляем пользователю НОВОЕ сообщение с кнопкой "Ввести код"
    ...
    # Отправляем НОВОЕ уведомление в канал (не редактируем старое)
    await bot.send_message(
        NOTIFY_CHANNEL_ID,
        f"🔑 Код запрошен (заявка {app_id}) - раз {new_count}"
    )

    # Обновляем сообщение у админа (в личке, чтобы он видел прогресс)
...
async def admin_cancel_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    # Редактируем только сообщение админа, канал не трогаем (уже отправили новое)
    storage_key = StorageKey(bot.id, user_id, user_id)
